...
public class Spawner : MonoBehaviour
{
    ...
    IEnumerator DelayedStart()
    {
        ...
        timer = spawnInterval;  // 바로 스폰하도록 초기화
    }
    ...
    void SpawnNormal(float currentTime)
    {
        ...
        GameObject enemy = GameManager.instance.pool.Get(0); // 일반 몬스터는 풀 인덱스 0
    ...
    void SpawnElite(int id)
    {
        ...
        if (data == null)
        {
            Debug.LogWarning($"[엘리트 몬스터] ID {id}에 해당하는 데이터가 없습니다.");
            return;
        }

        GameObject enemy = GameManager.instance.pool.Get(3); // 엘리트 몬스터는 풀 인덱스 3
        enemy.transform.position = spawnPoint[Random.Range(1, spawnPoint.Length)].position;
        ...
        enemy.GetComponent<Enemy>().Init(data);
        Debug.Log($"[엘리트 몬스터] {data.Name} (ID: {data.Id}) 등장");
    }
